...
row via `max(interestingness_score)` on the AggregatingMergeTree side. We
...
from posthog.temporal.session_replay.interestingness_scoring_sweep.constants import KAFKA_PRODUCE_FLUSH_TIMEOUT_S

ACTIVITIES_MODULE = "posthog.temporal.session_replay.interestingness_scoring_sweep.activities"
...
class TestPublishScores:
    ...
    def test_payload_carries_score_and_distinct_id(self, id_frame: pd.DataFrame) -> None:
        ...
        with mock.patch(f"{ACTIVITIES_MODULE}.get_producer") as get_producer_mock:
            ...
            assert [p["interestingness_score"] for p in payloads] == [

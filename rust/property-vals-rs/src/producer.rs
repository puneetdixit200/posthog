...
/// Lightweight snapshot of a consumer offset, owned (no consumer handle).
/// The worker stores these in `pending_offsets` so they can be passed to
/// `produce_and_commit` and committed atomically with the produce inside a
/// Kafka transaction. Using a snapshot type instead of common-kafka's
/// `Offset` keeps the trait constructible from tests.
#[derive(Debug, Clone)]

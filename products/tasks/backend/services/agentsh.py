...
def _port_from_url(url: str | None) -> int | None:
    ...
    try:
        ...
        # Non-numeric or out-of-range port in `SANDBOX_*_URL` (e.g. `http://host:abc`,
        # `:99999`). Tolerate silently rather than crash `generate_policy_yaml` —
        # a typo in `.env` should degrade to "no port added", not block sandbox boot.
        return None
    if port is not None:
        return port

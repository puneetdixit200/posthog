...
export function createInvocationSanitizer(config: Pick<CdpConfig, 'CDP_CYCLOTRON_STRIP_PERSON_FROM_STATE_TEAMS'>) {
    ...
    return {
        sanitizeInvocation(invocation: CyclotronJobInvocation): CyclotronJobInvocation {
            return sanitizeInvocationForPersistence(invocation, {
                stripPerson: stripPersonMatcher(invocation.teamId),
            })
        },
